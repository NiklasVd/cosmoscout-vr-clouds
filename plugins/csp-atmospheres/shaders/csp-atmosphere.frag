...
vec2 intersectSphere(vec3 rayOrigin, vec3 rayDir, float radius) {
  ...
  // Firstly, we find the coefficient t_a, which is the length , such that the position O + t_a * D = L along the ray OD, forms a
  // right triangle OCL.
  // Secondly, we calculate the coefficient t_b, which is the length of the right triangle, such that |CL|^2 + t_b^2 = r^2 (Pythagoras),
  // where r is the radius. Rearranging and taking the square root yields t_b.
  // => t_0 = t_a - t_b
  ...
  // Calculate length of ray from sphere entry to 
...
// Shifts the bounds of a value v
...
vec4 raymarchInterval(vec3 rayOrigin, vec3 rayDir, vec3 sunDir, vec2 interval, out vec3 path_transmittance) {
  ...
  float t_last = interval.x; // float t_last = interval.x;
  // progress is in [0, 1]
  float progress = 0;
  ...
  // Idea: sample transmittance to sun only every couple steps and interpolate the sampled values in between
...
// #if EXPERIMENTAL_CLOUD_FEATURES
//     // Increase step size with every step
//     step_size += samples_taken > 100 ? float(samples_taken) * small_step / CLOUD_QUALITY : 0;
// #endif
    progress += step_size;
    ...
    if(!CumuloNimbusGuaranteedFree(position)) {
      ...
      int SKY_LUMINANCE_INTERPOLATION_STRIDE = 20;
...
vec4 getCloudColor/*IntersectAndOct*/(vec3 rayOrigin, vec3 rayDir, vec3 sunDir, float surfaceDistance, out vec3 transmittance) {
  ...
  // Check where ray intersects top/ bottom cloud layer and atmosphere
  ...
  // => Returns coefficients t_0, t_1, such that rayOrigin + t_0 * rayDir = first intersection, ...
...
// Octree implemented instead of interval calculation
vec4 getCloudColorOnlyOctree(vec3 rayOrigin, vec3 rayDir, vec3 sunDir, float surfaceDistance, out vec3 transmittance) {
  float thickness = CUMULONIMBUS_END_HEIGHT - CUMULONIMBUS_START_HEIGHT;
  ...
  vec2 interval1 = vec2(0, -1);
  vec2 hitInterval;
  while (treeRaycast(rayOrigin + rayDir * (interval1.y + 1), rayDir, hitInterval)) {
    interval1 = vec2(min(interval1.x, hitInterval.x), max(interval1.y, hitInterval.y));
  }
  // Edge case: if ray pierces cloud layer from above, leaves it below and then
  // enters again from below along the bend of the planet, we calculate this interval, too. 
  vec2 interval2 = vec2(0, -1);
...
vec4 getCloudColorOld(vec3 rayOrigin, vec3 rayDir, vec3 sunDir, float surfaceDistance, out vec3 transmittance) {
  float thickness = CUMULONIMBUS_END_HEIGHT - CUMULONIMBUS_START_HEIGHT;
...
float getCloudShadow(vec3 rayOrigin, vec3 rayDir) {
  ...
  #else
  vec2 topIntersections = intersectSphere(rayOrigin, rayDir, PLANET_RADIUS + CUMULONIMBUS_END_HEIGHT);
  vec2 lowIntersections = intersectSphere(rayOrigin, rayDir, PLANET_RADIUS + CUMULONIMBUS_START_HEIGHT);
  ...
  float density = getCloudDensity(rayOrigin, rayOrigin).x;
  float transmittance = raymarchTransmittance(rayOrigin, rayDir, interval, rayOrigin, transmittance_samples, density, false);
...
// Default cloud model, not truly physically based
  ...
  if (!underWater) {
    ...
    oColor = oColorOld * transmittance + cloudColor.rgb; //oColorOld * transmittance + cloudColor.rgb;
    // oColor = oColorOld * transparency + cloudColor.rgb;
  }
#endif
#endif
